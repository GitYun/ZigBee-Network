...
const SimpleDescriptionFormat_t zb_SimpleDesc =
{
  ...
  (cId_t *) zb_InCmdList,     //  Input Command List, cId_t = uint16
...
void zb_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if ( shift )
  {
    ...
    if ( keys & HAL_KEY_SW_6 ) // 0x20, P0_1 
    {
  ...
  {
    if ( keys & HAL_KEY_SW_1 )  // 0x01, P2_0(S2)作为建立网络重启按键
    {
      if ( myAppState == APP_INIT  )
      {
        ...
        // Key 1 starts device as a coordinator
         ...
         // 将 logicalType 设置为协调器，并保存在 NV 中
        ...
        // 修改 StartOptions 的值， 并保存在 NV 中
        zb_ReadConfiguration( ZCD_NV_STARTUP_OPTION, sizeof(uint8), &startOptions );
        startOptions = ZCD_STARTOPT_AUTO_START; // 0x04
        zb_WriteConfiguration( ZCD_NV_STARTUP_OPTION, sizeof(uint8), &startOptions );
      
        zb_SystemReset(); // 重启协调器设备, 看门狗复位
      }
      else
      {
        // Initiate a binding
        zb_AllowBind( myAllowBindTimeout );     // 10
      }
    }
    if ( keys & HAL_KEY_SW_2 )  // 0x02
    {
//      if ( myAppState == APP_INIT )
//      {
//        // In the init state, keys are used to indicate the logical mode.
...
//      {
//      }
#pragma message("HAL_KEY_SW_2 被触发")
    }
    if ( keys & HAL_KEY_SW_3 )
    {
    }
...
void zb_AllowBindConfirm( uint16 source )
{
  ...
  //HalLedSet( HAL_LED_1, HAL_LED_MODE_BLINK );
	HalLedBlink (HAL_LED_1, 5, HAL_LED_DEFAULT_DUTY_CYCLE, HAL_LED_DEFAULT_FLASH_TIME);
}
/******************************************************************************
