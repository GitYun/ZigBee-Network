...
void zb_HandleOsalEvent( uint16 event )
{
  ...
  if ( event & MY_START_EVT )   // MY_START_EVT = 0x0001
  {
    zb_StartRequest();
  }
...
void zb_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if ( shift )  // ture if keys contains HAL_KEY_SW_6(P0_1), or false
  {
