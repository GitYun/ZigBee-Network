  ...
  else
  {
    ...
    if ( newTimer )
    {
      ...
      else
      {
        ...
        // Add to the list
        srchTimer->next = newTimer; // 加入新Timer到srchTimer，也更新了timerHead
      }
...
{
  halIntState_t intState;   // unsigned char 类型
  osalTimerRec_t *newTimer; // 结构体类型
...
uint8 osal_stop_timerEx( uint8 task_id, uint16 event_id )
{
  ...
  if ( foundTimer ) // 已存在的Timer，或者为NULL
  {
