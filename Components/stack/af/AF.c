...
epList_t *afRegisterExtended( endPointDesc_t *epDesc, pDescCB descFn )
{
  ...
  if ( ep )
  {
    ...
    ep->flags = eEP_AllowMatch; // 枚举量，1
    ep->pfnDescCB = descFn;     // 回调函数
    ep->nextDesc = NULL;        // 下一个端点描述符的指针
...
afStatus_t afRegister( endPointDesc_t *epDesc )
{
  ...
  if ( afFindEndPointDescList( epDesc->endPoint ) )
    return ( afStatus_INVALID_PARAMETER );  // 端点描述符已注册，则不再注册此描述符
  // 向端点描述符列表中添加当前的端点描述符，返回新列表条目
                           ...
                           uint8 options, uint8 radius )
{
  ...
  req.profileID = ZDO_PROFILE_ID;		// 0
	
	// 所有用afRegister()注册的终端描述符，在注册时，都没有设置回调函数，
	// 所以查找回调函数 afGetDescCB(srcEP) 返回 NULL。
	// 要使用回调函数，则注册终端描述符时使用 afRegisterExtended()
  if ( (pfnDescCB = afGetDescCB( srcEP )) )	// 注意这里是 "=" 而不是 "=="
  {
    uint16 *pID = (uint16 *)(pfnDescCB(
                                 AF_DESCRIPTOR_PROFILE_ID, srcEP->endPoint ));	// 2, ...
    if ( pID )
    {
      req.profileID = *pID;
      osal_mem_free( pID );
  ...
  else if ( srcEP->simpleDesc )	// 绑定时，不执行
  {
...
#if defined ( INTER_PAN )
  req.dstPanId      = dstAddr->panId;		// 绑定时，无 panId
...
uint8 afSetMatch( uint8 ep, uint8 action )
{
  ...
  if ( epSearch )
  {
    if ( action )
    {
      epSearch->flags |= eEP_AllowMatch;    // eEp_AllowMatch 枚举量，1
    }
    else
    {
