...
void ZDO_StartDevice( byte logicalType, devStartModes_t startMode, byte beaconOrder, byte superframeOrder )
{
  ...
  ret = ZUnsupportedMode;   // 0x12

  if ( ZG_BUILD_COORDINATOR_TYPE && logicalType == NODETYPE_COORDINATOR )
  {
    if ( startMode == MODE_HARD )
    {
      devState = DEV_COORD_STARTING;    // enum
      // 参数：0xFFFF（PAN ID默认值）,[00 ,.., 00]（8元素数据）, 0x00000800（channel 11）, 5, 0, 0, false
      // 此函数建立网络后，将给 ZDO 层发送反馈信息，
      // 即调用 ZDApp.c 中的 ZDO_NetworkFormationConfirmCB()
      
      // 此函数允许相邻的上层去请求设备组建一个新网络并允许自身成为该网络的ZigBee 协
      // 调器。这个行为的结果（状态）返回到ZDO_NetworkFormationConfirmCB() 回调函数中。
    ...
    else if ( startMode == MODE_RESUME )
    {
      ...
      // 这个函数允许上层去请求设备作为路由器启动，这个行为的结果（状态）返回到
      // StartRouterConfirmCB() 回调函数中
  ...
  if ( ZG_BUILD_JOINING_TYPE && (logicalType == NODETYPE_ROUTER || logicalType == NODETYPE_DEVICE) )
  {
    ...
    {
      devState = DEV_NWK_DISC;  // enum

  #if defined( MANAGED_SCAN )
      ZDOManagedScan_Next();
      
      // 请求网络层去发现相邻的路由器。这个函数应在加入之前调用，以执行一个网络
     // 扫描。扫描确认结果将被返回到ZDO_NetworkDiscoveryConfirmCB()回调函数中
      ret = NLME_NetworkDiscoveryRequest( managedScanChannelMask, BEACON_ORDER_15_MSEC );
  #else
      ret = NLME_NetworkDiscoveryRequest( zgDefaultChannelList, zgDefaultStartingScanDuration );// 11, 5
    #if defined ( ZIGBEE_FREQ_AGILITY )
    ...
    else if ( startMode == MODE_RESUME )
    {
      ...
      {
        devState = DEV_NWK_ORPHAN;
        // 此函数请求网络层孤儿加入网络。此函数是一个默认加入形式的扫描。此函数的结果（状态
        // 值）返回到ZDO_JoinConfirmCB() 回调函数中。
  ...
  if ( ret != ZSuccess )
    osal_start_timerEx(ZDAppTaskID, ZDO_NETWORK_INIT, NWK_RETRY_DELAY );  // 1000ms
}
...
void ZDO_UpdateNwkStatus(devStates_t state)
{
  epList_t *pItem = epList;
	...
	// 搜索端点列表，寻找之前在应用层注册过的端点号
  while (pItem != NULL)
  {
    if (pItem->epDesc->endPoint != ZDO_EP)  // ZDO_EP = 0，端点0
    {
			// 将 ZDO 状态改变消息发送给搜索在的端点
