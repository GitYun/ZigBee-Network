...
static byte ZDP_TxOptions = AF_TX_OPTIONS_NONE;		// 0
...
// 下面数组列表分别列出了使用 ZDO_RegisterForZDOMsg(task_ID, task_rsq) 注册任务要响应的事件后，
// 请求和响应的关系，一个请求对应一个处理函数指针
                                ...
                                byte SecurityEnable )
{
  ...
  {
    dstAddr->addr.shortAddr = NWK_BROADCAST_SHORTADDR_DEVRXON;	// 0xFFFD
  }
