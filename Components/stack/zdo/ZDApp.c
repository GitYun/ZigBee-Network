...
#define DEFAULT_BEACON_ORDER        BEACON_ORDER_NO_BEACONS // 15
#define DEFAULT_SUPERFRAME_ORDER    DEFAULT_BEACON_ORDER    // 15
...
// zAddrType_t 主要用于 ZigBee 设备的（地址）绑定
...
uint8 ZDApp_LeaveCtrl;
  ...
  devStartModes_t devStartMode = MODE_HARD; // 枚举量
#endif

uint8 retryCnt;

// 端点0，每个 ZigBee 设备必须使用的端点，用于寻址所有活动端点
endPointDesc_t ZDApp_epDesc =
{
  ZDO_EP,   // endPoint, 0
  &ZDAppTaskID, // task_id, 是 osalInitTasks() 中分配的任务ID
  (SimpleDescriptionFormat_t *)NULL,  // No Simple description for ZDO
  (afNetworkLatencyReq_t)0            // No Network Latency req
};

uint16 ZDApp_SavedPollRate = POLL_RATE;	// default value is 1000 ms
...
void ZDApp_Init( uint8 task_id )
{
  ...
  // Initialize the ZDO global device short address storage
	// zAddrType_t 主要用于 ZigBee 设备的（地址）绑定
  ZDAppNwkAddr.addrMode = Addr16Bit;    // 2, enum
  ...
  else
  {
    ...
    HalLedBlink ( HAL_LED_4, 5, 50, 500 );
  }
...
UINT16 ZDApp_event_loop( uint8 task_id, UINT16 events )
{
  ...
  if ( events & ZDO_NETWORK_INIT )  // 0x0001, 网络初始化事件处理
  {
    // Initialize apps and start the network
    devState = DEV_INIT;
    
    ZDO_StartDevice( (uint8)ZDO_Config_Node_Descriptor.LogicalType, devStartMode,   // ..., MODE_HARD
                     DEFAULT_BEACON_ORDER, DEFAULT_SUPERFRAME_ORDER );  // 15, 15
  ...
  {
    if ( events & ZDO_NETWORK_START )   // 0x0002, 新网络开始事件
    {
  ...
  // ZDApp_NetworkStartEvt() 或 ZDApp_ProcessNetworkJoin()，... 函数
  // 中设置的 ZDO_STATE_CHANGE_EVT ZDO 状态改变事件
  if ( events & ZDO_STATE_CHANGE_EVT ) // 0x0010
  {
    // 向 AF 层中已注册（在 aspi.c 中的 SPAI_Init()中使用 afRegister(&sapi_epDesc) 注册）
    // 的端点描述符中的 task_id（应该为 spai_TaskID）所标识的任务
    // 发送 ZDO_STATE_CHANGE 事件（此事件被附加在一个系统消息
    // 事件 SYS_EVENT_MSG 中）
    // 当设备加入网络时，devState 在 ZDApp_ProcessOSALMsg() 中改变状态值为 DEV_END_DEVICE
    // 当设备建立网络时，devState 在 ZDApp_NetworkStartEvt() 中改变状态为 DEV_ZB_COORD
    ZDO_UpdateNwkStatus( devState );
    
    // At start up, do one MTO route discovery if the device is a concentrator
    if ( zgConcentratorEnable == TRUE ) // 默认设置为 false
    {
      // Start next event
...
uint8 ZDOInitDevice( uint16 startDelay )
{
  ...
  // 如果在 IAR中定义了宏 HOLD_AUTO_START，
  // 则 devState 初始化时为 DEV_HOLD，否则，初始化为 DEV_INIT
  if ( devState == DEV_HOLD ) 
  {    
  ...
  ZDConfig_InitDescriptors();   // 初始化 ZDO 描述符，node  power
  ...
  if ( devState == DEV_HOLD )
  {
    ...
    zgWriteStartupOptions( ZG_STARTUP_SET, ZCD_STARTOPT_DEFAULT_NETWORK_STATE );	// 0xFF, 0x02
  ...
  if ( networkStateNV == ZDO_INITDEV_NEW_NETWORK_STATE )
  {
    ...
    extendedDelay = (uint16)((NWK_START_DELAY + startDelay)	// 100 + startDelay
              + (osal_rand() & EXTENDED_JOINING_RANDOM_MASK));	// random + 127
  }
...
void ZDApp_NetworkStartEvt( void )
{
  // 若网络形成函数 NLME_NetworkFormationRequest() 建立网络成功，
  // 则返回 zSuccess，否则返回 zFailure 并将此返回值传递给
  // ZDO_NetworkFormationConfirmCB() 函数，再在其中将上面的 
  // zSuccess 或 zFailure 赋值给 nwkStatus
  if ( nwkStatus == ZSuccess )  // 网络建立成功?
  {
    // Successfully started a ZigBee network
    // devState 在 ZDObject.c 中的 ZDO_StartDevice() 函数中
    // 改变状态
...
void ZDAppCheckForHoldKey( void )
{
  ...
  if ( HalKeyRead () == SW_BYPASS_START)    // 0x01
  {
...
void  ZDApp_ProcessOSALMsg( osal_event_hdr_t *msgPtr )
{
  ...
  switch ( msgPtr->event )
  {
    ...
    case AF_INCOMING_MSG_CMD:   // 0x1A, AF 层信息输入事件
      ZDP_IncomingData( (afIncomingMSGPacket_t *)msgPtr );
      break;
      
    case ZDO_CB_MSG:	// 0xD3
    ...
    case ZDO_NWK_DISC_CNF:  // 网络发现确认事件
      // devState 在 ZDObject.c 的 ZDO_StartDevice() 中改变状态
      ...
      if ( ZG_BUILD_JOINING_TYPE && ZG_DEVICE_JOINING_TYPE )
      {
        if ( (((ZDO_NetworkDiscoveryCfm_t *)msgPtr)->hdr.status == ZDO_SUCCESS) && (zdoDiscCounter > NUM_DISC_ATTEMPTS) )
        {
          if ( devStartMode == MODE_JOIN )
          {
            ...
            // 此函数允许上层去请求设备将自己加入到一个网络中。
            // 这个行为的结果（状态）返回到 ZDO_JoinConfirmCB() 回调函数中。
          ...
          else if ( devStartMode == MODE_REJOIN )
          {
            ...
            // 此函数重新加入一个设备已经加入过的网络。
            // 此函数的结果（状态）返回到 ZDO_JoinConfirmCB() 回调函数中。
      ...
      // 来自 ZDO_JoinConfirmCB() 函数中发送的 ZDO_NWK_JOIN_IND 事件
    case ZDO_NWK_JOIN_IND:  // 网络加入指示事件
      if ( ZG_BUILD_JOINING_TYPE && ZG_DEVICE_JOINING_TYPE )
      {
...
void ZDApp_ProcessNetworkJoin( void )
{
  // devState 在 ZDO_StartDevice() 函数中改变状态值
  if ( (devState == DEV_NWK_JOINING) ||
      ((devState == DEV_NWK_ORPHAN)  &&
       (ZDO_Config_Node_Descriptor.LogicalType == NODETYPE_ROUTER)) )
  {
    // Result of a Join attempt by this device.
    // nwkStatus 在 ZDO_JoinConfirmCB() 函数中改变状态值，
    // 实际来自于 NLME_JoinRequest() 函数的返回结果，此返回
    // 结果被反馈给了 ZDO_JoinConfirmCB()
    if ( nwkStatus == ZSuccess )
    {
      osal_set_event( ZDAppTaskID, ZDO_STATE_CHANGE_EVT );
...
void ZDO_JoinConfirmCB( uint16 PanId, ZStatus_t Status )
{
  ...
  // Status 为 NLME_JoinReques() 函数返回的结果；
  // 若加入网络成功，则为 zSuccess，否则，为 zFailure
  nwkStatus = (byte)Status;

  if ( Status == ZSUCCESS ) // 加入网络成功？
  {
