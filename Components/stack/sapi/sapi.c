...
void zb_StartRequest()
{
  ...
  {
    logicalType = ZB_SUCCESS;  // 0x00
    // 设备上电启动后，由于未形成网络，经逻辑类型判断后，执行此处
    ZDOInitDevice(zgStartDelay);    // zgStartDelay = 0x0a
  }
...
void zb_BindDevice ( uint8 create, uint16 commandId, uint8 *pDestination )
{
  ...
  uint8 ret = ZB_ALREADY_IN_PROGRESS;   // 0x20

  if ( create )
  {
    // 在 SAPI_Init() 中定义 sapi_bindInProgress = 0xffff
    if (sapi_bindInProgress == 0xffff) 
    {
      ...
      else
      {
        ...
        destination.addrMode = Addr16Bit;   // 2
        ...
        if ( ret == ZB_SUCCESS )
        {
          ...
          // zgApsDefaultMaxBindingTime = 16000
          osal_start_timerEx(sapi_TaskID, ZB_BIND_TIMER, zgApsDefaultMaxBindingTime);
#endif
          sapi_bindInProgress = commandId;  // 允许基于命令的绑定过程
          return; // dont send cback event 不发送回调事件
        }
      }
    }
    // 发送绑定确认事件到 APS 层
    SAPI_SendCback( SAPICB_BIND_CNF, ret, commandId );// SAPICB_BIND_CNF = 0xE1
  }
  else
  {
...
void zb_AllowBind ( uint8 timeout )
{
  ...
  osal_stop_timerEx(sapi_TaskID, ZB_ALLOW_BIND_TIMER);// ..., 0x4000
                          ...
                          uint8 *pData, uint8 handle, uint8 txOptions, uint8 radius )
{
  ...
  if (destination == ZB_BINDING_ADDR)   // 0xFFFE
  {
    // Binding
...
void SAPI_StartConfirm( uint8 status )
{
  ...
  {
#if ( SAPI_CB_FUNC )
    zb_StartConfirm( status );  // 改变应用运行状态标志
#endif
  }
}
 ...
 * @return      none
 */
// 允许绑定的设备（不是绑定请求的发起设备）接收到绑定请求且绑定设备后，
// 用于确定绑定请求已收到
...
UINT16 SAPI_ProcessEvent( byte task_id, UINT16 events )
{
  ...
  if ( events & SYS_EVENT_MSG ) // 消息事件， 0x8000
  {
    ...
    while ( pMsg )
    {
      switch ( pMsg->event )
      {
        case ZDO_CB_MSG:    // 0xD3, ZDO 层的反馈消息事件
          SAPI_ProcessZDOMsgs( (zdoIncomingMsg_t *)pMsg );
          break;

        case AF_DATA_CONFIRM_CMD:   // 0xFD，数据确认事件
        ...
        case AF_INCOMING_MSG_CMD:   // 0x1A，AF 层消息输入事件
        ...
        case ZDO_STATE_CHANGE:  // 0xD1, ZDO 网络状态改变事件 
  ...
  if ( events & ZB_ALLOW_BIND_TIMER ) // 0x4000，允许绑定超时事件
  {
		// 在 zb_AllowBind(uint8 timeout) 执行后的 timeout 秒内，
		// 没有绑定设备，则关闭匹配描述符
    afSetMatch(sapi_epDesc.simpleDesc->EndPoint, FALSE);
  ...
  if ( events & ZB_ENTRY_EVENT )    // 0x1000，进入事件
  {
    ...
    // 关闭在 ZDAPP.c 中 ZDAPP_Init() 中的 LED 闪烁
    HalLedSet (HAL_LED_4, HAL_LED_MODE_OFF);  // LED4 = LED1

    // 从设备的非易失性存储器中读取配置信息
    zb_ReadConfiguration( ZCD_NV_STARTUP_OPTION, sizeof(uint8), &startOptions );
    if ( startOptions & ZCD_STARTOPT_AUTO_START )   // 0x04
    {
      // 若设备是 Coordinator, 重启后，建立网络;
      // 否则，重启后，加入网络
      zb_StartRequest();    
    }
    else
    {
      ...
      HalLedBlink(HAL_LED_2, 10, 50, 500);   // HAL_LED_2 = 0x02
    }
...
void SAPI_ProcessZDOMsgs( zdoIncomingMsg_t *inMsg )
{
  switch ( inMsg->clusterID )
  {
    ...
    case Match_Desc_rsp:   // 匹配描述符消息的响应
      {
        zAddrType_t dstAddr;
        ZDO_ActiveEndpointRsp_t *pRsp = ZDO_ParseEPListRsp( inMsg );

        // 发送绑定请求时，已赋值：sapi_bindInProgress = Command ID
        if ( sapi_bindInProgress != 0xffff )
        {
...
{
  sapi_TaskID = task_id;    // 5 or 6 or 7 or 8
  sapi_bindInProgress = 0xffff; // 设置绑定标志位，默认不允许绑定

  // 初始化本设备用来通信的APS层端点描述符
...
//  sapi_epDesc.endPoint = 0;
  ...
  //使用端点描述符向AF层登记一个端点（终端）
  afRegister( &sapi_epDesc );   
#endif

  // Turn off match descriptor response by default
  // ZDO 不响应指定端点的 ZDO 匹配描述符请求
  afSetMatch(sapi_epDesc.simpleDesc->EndPoint, FALSE);

  // Register callback evetns from the ZDApp
  ZDO_RegisterForZDOMsg( sapi_TaskID, NWK_addr_rsp );   // MWK_addr_rsp = 0x8000, 簇ID
  ZDO_RegisterForZDOMsg( sapi_TaskID, Match_Desc_rsp ); // Match_Desc_rsp = 0x8006, 簇ID

#if ( SAPI_CB_FUNC )
#if (defined HAL_KEY) && (HAL_KEY == TRUE)
  // Register for HAL events
  RegisterForKeys( sapi_TaskID ); // 向本层(APS)注册（HAL）按键事件
...
#if defined( MT_TASK )          
  MT_TaskInit( taskID++ );  //      no
#endif
  
  APS_Init( taskID++ );     //      3
  
#if defined( ZIGBEE_FRAGMENATION )
  APSF_Init( taskID++ );    //      no
#endif
