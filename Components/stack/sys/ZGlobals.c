...
// 宏 DEFAULT_CHANLIST 在配置文件 f8wConfig.cf 中定义，
// 默认值：0x00000800，即通道 11
uint32 zgDefaultChannelList = DEFAULT_CHANLIST; 

// Default starting scan duration
uint8 zgDefaultStartingScanDuration = STARTING_SCAN_DURATION;   // 5

// Stack profile Id
// 宏 ZIGBEEPRO 在 IAR 中定义时，STACK_PROFILE_ID = 2，否则为 1
...
// Many-to-one values
uint8 zgConcentratorEnable = CONCENTRATOR_ENABLE;	// default false
...
uint16 zgApsDefaultMaxBindingTime = APS_DEFAULT_MAXBINDING_TIME;// 16000
...
// Configured PAN ID
// 宏 ZDAPP_CONFIG_PAN_ID 在配置文件 f8wConfig.cfg 中配置
// 默认配置为 0xFFFF，需要建立网络，则更改为其他的值
