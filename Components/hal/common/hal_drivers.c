...
uint16 Hal_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & HAL_LED_BLINK_EVENT )   // 0x0002, LED 闪烁事件
  {
  ...
  if (events & HAL_KEY_EVENT)   // 0x0001, 按键事件
  {
#if (defined HAL_KEY) && (HAL_KEY == TRUE)
    /* Check for keys */
    HalKeyPoll();   // 函数在 hal_key.c 中

    /* if interrupt disabled, do next polling */
    if (!Hal_KeyIntEnable)
    {
     /* 
      * 此函数的功能是经过 100 毫秒后，向 Hal_TaskID
      * 这个ID所标示的任务（也就是其本身）发送一个HAL_KEY_EVENT事件。
      * 这样一来，每经过100毫秒，Hal_ProcessEvent 这个事件处理函数都
      * 会至少执行一次来处理HAL_KEY_EVENT事件。
